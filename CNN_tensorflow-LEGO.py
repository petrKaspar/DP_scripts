...
x = tf.placeholder('float', [None, 320*240])
y = tf.placeholder('float')


def neural_network_model(data):
    hidden_1_layer = {'weights': tf.Variable(tf.random_normal([320*240, n_nodes_hl1])),
...
def convolutional_neural_network(x):
    weights = {
        ...
        'W_fc': tf.Variable(tf.random_normal([4 * 240*320, 1024])),
    ...
    fc = tf.reshape(conv2, [-1, 4 * 240*320])
